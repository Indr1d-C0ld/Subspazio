...
| **Admin: editor profilo giocatore** — handle, colore/stemma/motto, allineamento, protezione novizio da `/admin/gioco` (`op=profile`, audit `player.profile`) | fatto — 2026-09-13 |
| **Audit globale della piattaforma** — 12 reperti (1 critico, 1 alto, 6 medi, 4 bassi). Nessuna SQL injection, nessun XSS, auth/CSRF/upload/webroot solidi | fatto — 2026-09-15 |
| **Integrità economica** (reperti 01+02) — `App\Game\Wallet`: addebiti con guardia nella WHERE, scambi a due lati in transazione. Applicato a 13 file. Prima: 100 cr → 5 acquisti da 41 cr tutti consentiti, saldo −310 | fatto — 2026-09-15 |
| **Test di integrazione** — primo impianto del progetto (`tests/`, `php tests/run.php`): 42 verifiche su integrità economica e non-regressione. Righe sintetiche `__test_`, pulizia garantita da shutdown function | fatto — 2026-09-15 |
| **Clock isolato** (reperto 03) — `TickHealth::esegui()`: un lavoro rotto non ferma più i successivi (con rollback della transazione ereditata). Corsa degradata distinta, allarme una volta per episodio, salute del clock visibile in `/admin/gioco`. `TickHealth::gc()` pota `tick_runs` — intacca anche il reperto 05 | fatto — 2026-09-15 |
| **Movimento NPC** (reperto 04) — cache di processo in `Universe` + letture raggruppate (`sectorsMany`, `warpsFromMany`) + scrittura unica con `CASE`. Da 393 query a 2; tick con 91 NPC da 343 ms di media storica a 32 ms. Il peggiore di sempre era 95.449 ms, con 120 s di buco il 2026-09-06 | fatto — 2026-09-15 |
| **Indici sulle query ricorrenti** (reperto 06) — migrazione `0034`: `npcs.last_move_at`, `players.last_seen_at`, `trade_log.created_at`, `combat_log.created_at`. Da `type=ALL` a `type=range`, verificato con EXPLAIN. Escluse dopo verifica `messages.created_at` (mai filtrata) e `players`/`planets.created_at` (solo FedNews, 1 volta al giorno) | fatto — 2026-09-15 |
| **Retention log** (reperto 05, parte finale) — **valutata e scartata**: `Achievements` conta «scambi» e «ferrengi» sull'intera storia, `BattleLog` espone le ultime 40 battaglie senza limite di tempo. Potare revocherebbe traguardi e cancellerebbe replay per recuperare 48+64 KB. Pericolo annotato nei due punti dove verrebbe introdotto | deciso — 2026-09-15 |
...
| **Audit delle meccaniche** (2026-09-19) — 5 reperti. La lezione di `Wallet` (vincolo nella WHERE **e** verifica che abbia morso) era stata applicata ai soli crediti: turni, giacimenti a colpo singolo e warp erano rimasti al vecchio schema. Provato in esecuzione: warp gratis con comandante e nave in settori diversi, turni a −1, un deposito pagato 3 volte. Tutti chiusi, 18 verifiche nuove in `tests/concorrenza.php` | fatto — 2026-09-19 |
| **Audit totale** (2026-09-23) — sette letture parallele del codice su tutte le aree mai scandagliate, poi ogni segnalazione verificata facendola accadere. Circa cinquanta difetti corretti: il mercato nero come stampante di crediti (+87% a giro, zero turni), interessi bancari retroattivi (+1,6 milioni su un versamento), contratti e lavori d'Officina rimborsati tre volte, caccia che sparivano negli assalti planetari, navi con zero caccia invulnerabili, colonie e porti che non ricrescevano, moduli promessi mai consegnati, cacciatori di taglie senza tetto, banditi in classifica, 205 battaglie finte nel registro. Quattro scelte di disegno decise con l'autore. Suite da 261 a 386 verifiche | fatto — 2026-09-23 |
